...
export const PropertyMediaCard = ({ unitId, history }: PropertyMediaCardProps) => {
  ...
  const unitMedia = unitData?.data?.media;
  const images = unitMedia?.images ?? [];
  const videos = unitMedia?.videos ?? [];
  ...
  const handleUpload = async () => {
    ...
    try {
      if (stagedImages.length > 0) {
        await uploadImages({ unitId, files: stagedImages }).unwrap();
      }
      if (stagedVideo) {
        await uploadVideo({ unitId, file: stagedVideo }).unwrap();
      }
  ...
  const handleUpdate = async () => {
    if (!unitId) return;
    if (stagedImages.length === 0 && !stagedVideo) {
      toast({ title: 'Nothing selected', description: 'Pick at least one image or video to replace existing media.', variant: 'destructive' });
      return;
    }
          ...
          {unitLoading ? (
            ...
            <div className="space-y-6">
              ...
              {images.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-medium">Property Images ({images.length})</span>
                    ...
                    {images.map((img) => (
                      <div key={img.publicId} className="relative group">
                        <img
                          ...
                          className="w-full h-20 object-cover rounded border hover:border-primary transition-colors cursor-pointer"
                          onClick={() => setLightboxUrl(img.url)}
                        />
                        <div className="absolute inset-0 bg-black/20 rounded opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
              ...
              {videos.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-medium">Property Videos ({videos.length})</span>
                    <Badge variant="secondary" className="text-xs">Unit</Badge>
                  </div>
              ...
              {(historyPhotos.length > 0 || historyVideos.length > 0) && (
                <div className={images.length > 0 || videos.length > 0 ? 'border-t pt-6' : ''}>
                  <div className="flex items-center gap-2 mb-3">
                    <Badge variant="outline" className="text-xs">Move-in Documentation</Badge>
                  </div>
                  {historyPhotos.length > 0 && (
                    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mb-3">
                      {historyPhotos.map((p) => (
                        <div key={p.public_id} className="relative group cursor-pointer">
                          <img
                            src={p.url}
                            alt="Move-in"
                  ...
                  {historyVideos.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {historyVideos.map((v) => (
                        <div key={v.public_id} className="relative">
                          <video src={v.url} className="w-full h-40 object-cover rounded border" controls />
              ...
              {images.length === 0 && videos.length === 0 && historyPhotos.length === 0 && historyVideos.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  <Image className="h-12 w-12 mx-auto opacity-40 mb-2" />
                  <p className="text-sm">No property media uploaded yet.</p>
                </div>
              )}
            ...
            {dialogMode === 'update' && (
              <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20 p-3">
                ...
                <p className="text-sm text-amber-800 dark:text-amber-300">
                  This will <strong>replace all existing media</strong> on this unit. Current images and videos will be permanently removed from Cloudinary.
                </p>
              </div>
            )}
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                  {imagePreviews.map((src, i) => (
                    <div key={i} className="relative group">
                      <img src={src} alt="" className="w-full h-16 object-cover rounded border" />
                      <button
            ...
            <div className="space-y-2">
              ...
              {videoPreview && (
                <div className="relative group w-full">
                  <video src={videoPreview} className="w-full h-32 object-cover rounded border" muted />
